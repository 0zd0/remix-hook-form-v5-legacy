...
export const stringRequired = () => z.string(errorMessages).trim().nonempty('validation.required')
...
export const passwordRequired = () => stringRequired().min(8, 'validation.min8')
...
export const generateObjectSchema = <T extends ZodRawShape>(obj: T) => z.object(obj)

export const generateObjectArraySchema = (obj: Record<string, z.ZodSchema>, required?: boolean) =>
    required ?
        z.array(generateObjectSchema(obj)).min(1, 'validation.required')
    :   z.array(generateObjectSchema(obj))

export const confirmEmail = <T extends { email: string; confirmEmail: string }>(data: T) =>
    data.email === data.confirmEmail

export const confirmPassword = <T extends { password: string; confirmPassword: string }>(data: T) =>
